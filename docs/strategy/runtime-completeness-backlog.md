...
This is the prioritized implementation backlog for the runtime extensions decided in VIGOR-7724. The seeds below are the work that lands the four ADRs (0031–0034) plus the tactical follow-ups identified in the strategic summary (`docs/strategy/runtime-completeness.md`).

## Summary table

| Seed ID | Priority | Title | ADR / strategic-doc reference |
| --- | :---: | --- | --- |
| [VIGOR-f497](#vigor-f497) | P0 | Implement parallel best-of-N (Budgets.parallel_candidates + asyncio.gather fanout) | ADR-0031; runtime-completeness §Q1 |
| [VIGOR-aa1c](#vigor-aa1c) | P0 | Add advisory archive lock + ArchiveLockedError (single-node enforcement) | ADR-0032; runtime-completeness §Q2 |
| [VIGOR-fb79](#vigor-fb79) | P0 | Iteration checkpoint + Orchestrator.resume(run_id) | ADR-0033; runtime-completeness §Q3 |
| [VIGOR-d8a3](#vigor-d8a3) | P0 | RuntimeObserver Protocol seam at lifecycle boundaries | ADR-0034; runtime-completeness §Q5 |
...
| [VIGOR-7086](#vigor-7086) | P1 | Orchestrator.run_streaming async-iterator entry point | runtime-completeness §Q6; ADR-0034 |
| [VIGOR-1029](#vigor-1029) | P2 | Harness backpressure / queue management for batched eval | runtime-completeness §Q8 |
| [VIGOR-c09b](#vigor-c09b) | P2 | Adopt atomic write-then-rename across all RunArchive writes | ADR-0033 follow-up |
| [VIGOR-c2ec](#vigor-c2ec) | P3 | Detect concurrent in-process Orchestrator.run on same archive | ADR-0032 follow-up |

**Priority breakdown:** 4× P0, 3× P1, 2× P2, 1× P3 (10 total).

## Why P0 vs P1 vs P2 vs P3

- **P0 — runtime-shape extensions ADR-committed.** Each P0 implements a foundational ADR (0031–0034) decided in this branch. The strategic doc classifies them all `foundational`; without them, VIGOR-the-library cannot credibly be called production-grade for v1.0. All four are independent of each other (no inter-P0 dependency), so a coordinator can parallelize them across builders.
- **P1 — known-shape work that extends an already-decided ADR.** Per-candidate Usage extends ADR-0028 (already proposed in sibling branch). Cancellation propagation and partial-result streaming extend ADR-0034's observer surface. Each has a clear ADR anchor; each is mechanical. None of them require a new ADR.
- **P2 — operator-friendly polish.** Backpressure for batched eval is harness-layer work, not runtime work; archive atomicity is a quality improvement that lifts ADR-0033's checkpoint guarantee to all archive writes. Both can ship in v1.1 without compromising the production-readiness story.
- **P3 — defensive guardrail.** In-process concurrent-run detection is a footgun-prevention addition the ADR-0032 advisory lock does not cover (advisory locks are per-process on POSIX). Plausibly worth adding; nobody is currently asking.

## What is intentionally NOT in this backlog

- **Distributed orchestration / cross-process locking.** Per ADR-0032, deferred to a future hosted `vigor-server`. Filing a Seeds task today would invite the very work the ADR is designed to defer.
- **Process-pool fanout for evaluation.** Per ADR-0031 Alt-A, rejected. In-tree adapters either subprocess (Manim, OpenSCAD) or are I/O-bound (rawpy, photo critics); a process pool is the wrong primitive.
- **Per-candidate checkpointing.** Per ADR-0033 Alt-A, rejected for v1. May revisit if operators ask; nobody has yet.
- **OpenTelemetry hard dependency.** Per ADR-0034 Alt-A, rejected. The `vigor-observability-otel` package is downstream — it can be filed there if/when it lands.
- **Prometheus metrics in `vigor-runtime`.** Per ADR-0034 and `docs/strategy/deployment-and-ops.md`, metrics are a deployment-layer concern. Not in the runtime.
- **Audit log instrumentation.** Already filed as VIGOR-a171 (sibling branch deployment backlog). Distinct from observability per ADR-0034.

## Seed details

### VIGOR-f497

**P0 — Parallel best-of-N.** Implements ADR-0031. Filed 2026-05-15.
...
**P0 — Advisory archive lock.** Implements ADR-0032. Filed 2026-05-15.
...
**P0 — Iteration checkpoint and resume.** Implements ADR-0033. Filed 2026-05-15.
...
8. Documentation: ADR-0033 reference in README; CLI `--help` documents resume.
...
**P0 — RuntimeObserver Protocol seam.** Implements ADR-0034. Filed 2026-05-15.
...
3. Wire the seven emission sites (per ADR-0034 §Decision Outcome). Each call wrapped in `try / except Exception` that logs to `logging.getLogger("vigor.runtime")` at WARNING and continues.
...
8. Documentation: ADR-0034 reference; example observer in `docs/examples/observer.py` (a simple stdout printer).
...
4. The observer's `on_candidate_end` (per ADR-0034) receives the per-candidate `Usage` as an attribute.
...
4. The streaming surface composes with parallel best-of-N (ADR-0031): events for parallel candidates emit as they complete, not in submission order. Document this.
...
**P2 — Atomic archive writes.** ADR-0033 follow-up. Filed 2026-05-15.
...
**P3 — In-process re-entrance detection.** ADR-0032 follow-up. Filed 2026-05-15.
...
**Why P3:** Programming-error case. The ADR-0032 advisory lock catches the cross-process case (the dominant failure mode); the in-process case requires a programmer to write `asyncio.gather(agent.run(t1), agent.run(t2))` which is not a documented pattern.
...
- **ADRs (this branch):**
  - `docs/adr/0031-parallel-best-of-n-via-asyncio-gather.md`
  - `docs/adr/0032-single-node-orchestrator-posture.md`
  - `docs/adr/0033-iteration-checkpoint-resume.md`
  - `docs/adr/0034-runtime-observer-protocol-seam.md`
- **ADRs (sibling branches, referenced):**
  - `docs/adr/0028-cost-ceiling-enforcement.md` (cost ceiling — VIGOR-ca10 extends its impl)
  - `docs/adr/0029-multi-tenant-subprocess-env-hardening.md` (referenced by ADR-0032 for hosted-layer coordination context)
  - `docs/adr/0030-library-first-deployment-posture.md` (foundational commitment that ADRs 0031–0034 all design against)
- **Strategy docs:**
...
- **Sibling Seeds (referenced):**
  ...
  - VIGOR-a171 (audit log — distinct from VIGOR-d8a3 per ADR-0034)
