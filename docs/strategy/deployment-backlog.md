...
| --- | :---: | --- | --- |
| [VIGOR-344f](#vigor-344f) | P0 | Implement Budgets.max_cost_usd enforcement (RunBudgetTracker + AgentBackend.usage + StopReason='cost_exceeded') | ADR-0012; scout §3 / rec #1 |
| [VIGOR-6468](#vigor-6468) | P0 | Harden MCP stdio subprocess env (drop-all default unless spec.env lists vars) | ADR-0013; scout §2 + §5 / rec #4 |
...
| [VIGOR-b2fc](#vigor-b2fc) | P2 | Add Dockerfile (multi-stage uv build) for vigor-agent CPU orchestrator | ADR-0019; ext §A / rec #7 |
...
- **P0 — paper-budget / leak surfaces.** Three documented commitments
  ...
  unwired. ADR-0012, ADR-0013, and ADR-0016 §3.2 are the corresponding
  written commitments.
...
- **P2 — operator-facing infrastructure.** Docker images and test-harness
  cleanups. Library-first per ADR-0019: VIGOR ships these as
...
- **`packages/vigor-server` HTTP wrapper.** Per ADR-0019, hosted hosting is
  an explicitly deferred downstream concern. A skeleton "experimental"
  service is the worst of both worlds (per ADR-0019 Alt-A). If a future
  ADR supersedes ADR-0019 to commit to a hosted service, that ADR will
  open the corresponding Seeds work.
- **Container sandbox per task** (Docker / nsjail / Firecracker per MCP
  server). Per ADR-0013 Alt-A and `docs/readiness/implementation-readiness.md`
...
- **Per-tenant authentication, OAuth2 / OIDC.** Depends on a hosted entry
  point that does not exist (ADR-0019). Threat-model surface only.
...
`StopReason="cost_exceeded"` literal. Per ADR-0012. Anchors:
...
default-change for any spec relying on inherit-all. Per ADR-0013.
...
`vigor.provenance.v1`, capturing tool calls, tenant ID (per ADR-0013),
...
Per ADR-0019, optional operator infra, not a hosted service.
...
- **ADRs (this branch):** `docs/adr/0012-cost-ceiling-enforcement.md`,
  `docs/adr/0013-multi-tenant-subprocess-env-hardening.md`,
  `docs/adr/0019-library-first-deployment-posture.md`.
